...
def system_startup(config: DictConfig):
    # CUDA_VISIBLE_DEVICES is now set early in main.py before PyTorch import
    # No need to set it here as it's too late
    ...
    # Handle cross-silo FL mode
    if config.get("cross_silo", False):
        # Validate cross-silo configuration
        ...
        # Override num_clients and num_clients_per_round for cross-silo
        config.num_clients = config.cross_silo_num_clients
        config.num_clients_per_round = config.cross_silo_num_clients  # All clients participate every round
...
def set_attack_config(config: DictConfig):
    # Handle cross-silo mode: use cross_silo_num_attackers instead of fraction_adversaries
    if config.get("cross_silo", False):
        num_attackers = config.cross_silo_num_attackers
        # Select attackers deterministically (first N clients) for reproducibility
        ...
        # Define malicious clients based on selection method (original logic)
