...
class ClientManager:
    ...
    def __init__(self, config, benign_client_class, malicious_client_class, start_round=0):
        ...
        if self.cross_silo_mode:
            ...
            # Store fixed client lists for cross-silo
        ...
        if config.no_attack:
            ...
            # Store fixed client lists for cross-silo mode
            if self.cross_silo_mode:
    ...
    def _initialize_poison_rounds(self):
        ...
        # Check if cross-silo mode is enabled
        if self.cross_silo_mode:
    ...
    def _initialize_normal_rounds(self):
        ...
        for r in range(self.start_round, self.start_round + self.config.num_rounds):
            if r not in self.poison_rounds:
                if self.cross_silo_mode:
                    # Cross-silo: use fixed clients (all clients participate)
    ...
    def _cross_silo_selection(self):
        ...
        # In cross-silo mode, all clients participate every round
        # Fixed malicious and benign clients are already set in __init__
